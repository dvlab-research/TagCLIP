...
class CLIPVisionTransformer(nn.Module):
    def __init__(self, input_resolution=224, patch_size=32, width=768, layers=12, heads=12, output_dim=512, 
        drop_path_rate=0.0, out_indices=[3, 5, 7, 11], pretrained=None, image_size=224, get_embeddings=False, **kwargs):
        super().__init__()
        ...
        if isinstance(pretrained, str):
            ...
            if 'positional_embedding' in state_dict.keys():
                if self.positional_embedding.shape != state_dict['positional_embedding'].shape:
                    ...
                    spatial_pos = F.interpolate(state_dict["positional_embedding"][1:,].reshape(1, self.image_size//self.patch_size, self.image_size//self.patch_size, self.embed_dim).permute(0, 3, 1, 2), size=(self.spatial_size, self.spatial_size), mode='bilinear')
                    spatial_pos = spatial_pos.reshape(self.embed_dim, self.spatial_size*self.spatial_size).permute(1, 0)
...
class VPTCLIPVisionTransformer(nn.Module):
    def __init__(self, input_resolution=224, patch_size=32, width=768, layers=12, heads=12, output_dim=512, drop_path_rate=0.0, out_indices=[3, 5, 7, 11], pretrained=None, get_embeddings=False, 
                 num_tokens=20, prompt_dim=512, total_d_layer=11, image_size=224, **kwargs):
        super().__init__()
        self.pretrained = pretrained
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        if isinstance(pretrained, str):
            ...
            if 'positional_embedding' in state_dict.keys():
                if self.positional_embedding.shape != state_dict['positional_embedding'].shape:
                    ...
                    print(f'Resize the pos_embed shape from {state_dict["positional_embedding"].shape} to {self.positional_embedding.shape}')
                    cls_pos = state_dict["positional_embedding"][0:1, :]
                    spatial_pos = F.interpolate(state_dict["positional_embedding"][1:,].reshape(
                        1, self.image_size//self.patch_size, self.image_size//self.patch_size, self.embed_dim).permute(0, 3, 1, 2), 
    ...
    def forward(self, x: torch.Tensor):
        # x: [1, 3, 512, 512] -> [1, 768, 32, 32]
        # import pdb;pdb.set_trace()
        x = self.conv1(x)
        B, C, H, W = x.shape
        # x: [1, 768, 32, 32] -> [1, 768, 1024] -> [1, 1024, 768]
        x = x.reshape(x.shape[0], x.shape[1], -1)
        x = x.permute(0, 2, 1)
        # add position embedding
        # x: [1, 1024, 768] -> [1, 1025, 768]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)
        ...
        # outs: [tuple(features), global_embedding]
        return outs
