    ...
    for i, (batch_indices, data) in enumerate(zip(loader_indices, data_loader)):
        ...
        if show or out_dir or out:
            ...
            for img, img_meta in zip(imgs, img_metas):
                ...
                if out:
                    pkl_file = os.path.join(out, img_meta['ori_filename'].split('.')[0]+'.pkl')
                    # print(f'\nwriting results to {pkl_file}')
                    mmcv.dump(result, pkl_file)
...
def main():
    ...
    if not distributed:
        ...
        results = single_gpu_test(model, data_loader, args.out, args.show, args.show_dir,
    ...
    # results = [torch.zeros((480, 640)) for _ in data_loader]
