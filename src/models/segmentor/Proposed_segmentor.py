...
class ProposedCLIPSegmentor(EncoderDecoder):
    ...
    def __init__(self,
                ...
                #  init_cfg=None,
                 **args):
        super(ProposedCLIPSegmentor, self).__init__(**args)
        ...
        self.discriminate_map = self._get_discriminate_map(self.base_class, self.novel_class, self.both_class)
    ...
    def _visiable_mask_st(self, seen_classes, novel_classes, both_classes):
        seen_map = np.array([255]*256) ### change -1 here to 255 to mix background and unseen class
        seen_map[255] = 255
        for i,n in enumerate(list(seen_classes)):
            seen_map[n] = n
    ...
    def extract_feat(self, img):
        ...
        # self.backbone: models.backbone.img_encoder.VPTCLIPVisionTransformer
    ...
    def encode_decode(self, img, img_metas):
        ...
        out = self._decode_head_forward_test(feat, img, img_metas, self.self_training)
        out = resize(
            input=out,
            size=img.shape[2:],
            mode='bilinear',
    ...
    def _decode_head_forward_test(self, x, img, img_metas, self_training):
        """Run forward function and calculate loss for decode head in
        inference."""
        seg_logits = self.decode_head.forward_test(x, img, img_metas, self.test_cfg, self_training)
        return seg_logits

    def _get_discriminate_map(self, seen_classes, novel_classes, both_classes):
        # discriminate_map = np.array([-1]*256)
        # discriminate_map[seen_classes] = 0
        # discriminate_map[255] = 0
        # discriminate_map[novel_classes] = 1
        # discriminate_map = {
        #     'seen_classes': seen_classes, 
        #     'novel_classes': novel_classes, 
        #     'both_classes': both_classes,
        # }
        discriminate_map = [seen_classes, novel_classes, both_classes]
        return discriminate_map

    def forward_train(self, img, img_metas, gt_semantic_seg):
        # visual_feat (list): [[1, 1, 512, 32, 32], [1, 512]]
        visual_feat = self.extract_feat(img)
        if self.load_text_embedding:
            # text_feat: [171, 512]
            text_feat = np.load(self.load_text_embedding)
            ...
            if not self.multi_prompts:
                ...
                raise AttributeError("preparing the multi embeddings")

        if not self.self_training:
            # text_feat: [156, 512]
            text_feat = text_feat[self.base_class, :]

        # feat: [visual_feat, text_feat]
        # visual_feat (list): [[1, 1, 512, 32, 32], [1, 512]]
        # text_feat         : [171, 512]
        feat = []
        feat.append(visual_feat)
    ...
    def _decode_head_forward_train(self, feat, img_metas, gt_semantic_seg):
        """Run forward function and calculate loss for decode head in
        training."""
        gt_semantic_seg_unmasked = gt_semantic_seg.clone()
        if self.training:
            if len(self.base_class) != len(self.both_class): # zero setting
                # [1, 1, 512, 512]
                # mask the unseen class
                gt_semantic_seg = torch.Tensor(self.visibility_seen_mask).type_as(gt_semantic_seg)[gt_semantic_seg]

        gt = dict(gt_semantic_seg=gt_semantic_seg, gt_semantic_seg_unmasked=gt_semantic_seg_unmasked, discriminate_map=self.discriminate_map)
        losses = dict()
        # self.decode_head: models.decode_heads.decode_seg.ATMSingleHeadSeg
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                # crop_seg_logit: [1, 15, 512, 512]
                crop_seg_logit = self.encode_decode(crop_img, img_meta)
                # [1, 20, 512, 699] += [1, 15, 512, 699]
